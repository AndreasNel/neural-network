...
#
# mnist = input_data.read_data_sets("datasets/", one_hot=True) # multiclass classification
...
def neural_network_model(data):
    ...
    #
    # l3 = tf.add(
    #     tf.matmul(l, hidden_3_layer["weights"]), hidden_3_layer["biases"])
    # l3 = tf.nn.relu(l3)
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(hm_epochs):
            ...
            while i < len(data["train_features"]):
                ...
                # print(len(batch_y))
                # print(len(batch_y[0]))
                # raise Exception('alkhsfdalkhjafd')
                i += batch_size
                # c is the cost
